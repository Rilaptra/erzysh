...
const LABELS = Object.fromEntries(Object.keys(COLORS).map((k) => [k, COLORS[k as LogType](k.toUpperCase())]));
...
function getCaller(): string {
  ...
  if (match?.[1]) {
    ...
    return !name || name === "Object" || name === "<anonymous>" || name.includes("ts:") ? "SYSTEM" : name;
  }
  return "SYSTEM";
}
...
export class ZyLog {
  ...
  private dispatch(type: LogType, message: any[], progress?: ProgressOptions) {
    ...
    if (!this.config.noContext) {
      ...
      contextStr = this.config.scope ? chalk.green.bold(`[${contextRaw}]`) : chalk.magenta.bold(`[${contextRaw}]`);
    }
    ...
    if (progress) {
      ...
      // String Builder
      const bar = color("━".repeat(filled)) + chalk.gray("━".repeat(length - filled));
