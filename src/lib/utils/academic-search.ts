...
function cariKegiatan(
  ...
  const { kegiatan, tanggal_mulai, tanggal_selesai, tanggal_angka, tahun, bulan, hari, clearCache = false } = options;

  // --- Sub-routines (Fungsi dalam fungsi) ---
  const parseTanggal = (tanggalStr: string): { date?: Date; tahun?: number; bulan?: number; tanggal?: number } | null => {
    if (/^\d{4}-\d{2}-\d{2}$/.test(tanggalStr)) {
      const date = new Date(tanggalStr);
      if (isNaN(date.getTime())) return null; // Validasi tanggal
      return { date, tahun: date.getFullYear(), bulan: date.getMonth() + 1, tanggal: date.getDate() };
  ...
  const validateEnum = (input: string | number, enumType: any, maxValidValue: number): number | null => {
    if (typeof input === 'number') {
        ...
        if (value !== undefined && value >= 0 && value <= maxValidValue) return value;
        else return null;
    }
    return null;
  };

  const isRangeMatch = (itemStart: Date, itemEnd: Date, filter: any): boolean => {
    ...
    if (filter.tahun !== undefined && filter.bulan !== undefined) {
        return (
            ...
            (itemThnStart === filter.tahun && itemThnEnd === filter.tahun && itemBlnStart <= filter.bulan && itemBlnEnd >= filter.bulan) ||
            (itemThnStart === filter.tahun && itemThnEnd > filter.tahun && itemBlnStart <= filter.bulan) ||
            (itemThnStart < filter.tahun && itemThnEnd === filter.tahun && itemBlnEnd >= filter.bulan)
        );
    }
  ...
  const bulanIndex = bulan !== undefined ? validateEnum(bulan, BulanEnum, 12) : null;
  ...
  const tglAngkaFilter = tanggal_angka !== undefined ? (tanggal_angka >= 1 && tanggal_angka <= 31 ? tanggal_angka : null) : null;
  ...
  // --- Cache ---
  const cacheKey = JSON.stringify({ dataRef: dataKalender, params: { kegiatan, tanggal_mulai, tanggal_selesai, tanggal_angka, tahun, bulanIndex, hariIndex } });
  ...
  const hasil = semuaKegiatan.filter((item) => {
    ...
    if (kegiatan && !item.kegiatan.toLowerCase().includes(kegiatan.toLowerCase())) return false;

    // Filter Rentang Tanggal (Mulai & Selesai)
    if (filterMulai && !isRangeMatch(itemStart, itemEnd, filterMulai)) return false;
    if (filterSelesai && !isRangeMatch(itemStart, itemEnd, filterSelesai)) return false;
    ...
    if (tglAngkaFilter) {
        ...
        let cocok = false;
        while (current <= itemEnd) {
            if (current.getDate() === tglAngkaFilter) { cocok = true; break; }
            current.setDate(current.getDate() + 1);
        }
    ...
    // Filter Tahun Global
    if (tahun !== undefined && (itemStart.getFullYear() > tahun || itemEnd.getFullYear() < tahun)) return false;
    ...
    if (bulanIndex !== null) {
        ...
        if (thn === undefined) {
             ...
             if (!( (itemThnStart === thn && itemBlnStart === bulanIndex) ||
                    (itemThnEnd === thn && itemBlnEnd === bulanIndex) ||
                    (itemThnStart < thn && itemThnEnd > thn) ||
                    (itemThnStart === thn && itemThnEnd === thn && itemBlnStart <= bulanIndex && itemBlnEnd >= bulanIndex) ||
                    (itemThnStart === thn && itemThnEnd > thn && itemBlnStart <= bulanIndex) ||
                    (itemThnStart < thn && itemThnEnd === thn && itemBlnEnd >= bulanIndex) )) return false;
        }
    }
    ...
    if (hariIndex !== null) {
      ...
      let cocok = false;
      while (current <= itemEnd) {
        if (current.getDay() === hariIndex) { cocok = true; break; }
        current.setDate(current.getDate() + 1);
      }
      if (!cocok) return false;
    }
