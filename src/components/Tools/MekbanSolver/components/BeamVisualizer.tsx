...
const BeamVisualizer: React.FC<Props> = ({ config, isDarkMode = false }) => {
  ...
  return (
    <div className="flex w-full flex-col items-center overflow-x-auto pb-4">
      {/* Wrapper to ensure minimum width for readability */}
      <div className="min-w-[800px]">
        <svg width={width} height={height} viewBox={`0 0 ${width} ${height}`}>
          ...
          {config.supports.map((s) => (
            <g
              key={s.id}
              transform={`translate(${scaleX(s.position)}, ${beamY + 5})`}
            >
            ...
            if (l.length) {
              ...
              if (l.type === LoadType.DISTRIBUTED) {
                return (
                  <g
                    key={l.id}
                    transform={`translate(${xStart}, ${beamY - 5})`}
                  >
                    <rect
                      x="0"
                      y="-30"
