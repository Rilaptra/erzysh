...
const TOKEN =
  ...
  'PASTE_TOKEN_TELEGRAM_LO_DISINI_KALAU_ENV_GAK_KEBACA'

const DOMAIN = isDev
  ? 'https://38a2f86a3dff.ngrok-free.app'
  : 'https://erzysh.vercel.app'
const SECRET = process.env.TELEGRAM_SECRET_TOKEN // Samain sama TELEGRAM_SECRET_TOKEN di .env
async function main() {
  if (
    !TOKEN ||
    TOKEN.length < 10 ||
    TOKEN === 'PASTE_TOKEN_TELEGRAM_LO_DISINI_KALAU_ENV_GAK_KEBACA'
  ) {
  ...
  try {
    ...
    console.log(
      `🔑 Secret Token: ${SECRET} (Jangan lupa taruh di .env: TELEGRAM_SECRET_TOKEN)`,
    )
  } catch (e) {
